...
## Smart Question 3: How do rental prices differ based on the source of the listing and can we identify any specific trends associated with each source?
...
# Summary statistics
summary(data)
# Initial boxplot to visualize outliers
ggplot(data, aes(x = source, y = price, fill = source)) + 
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Initial Rental Prices by Source", x = "Source", y = "Price") +
...
# Filter out outliers more stringently
data_no_outliers <- data %>%
  filter(price > lower_bound & price < upper_bound)

# Summary to check the effect of outlier removal
summary(data_no_outliers$price)

# Replot boxplot without outliers
ggplot(data_no_outliers, aes(x = source, y = price, fill = source)) + 
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Rental Prices by Source (Without Outliers)", x = "Source", y = "Price") +
  scale_fill_brewer(palette = "Spectral")

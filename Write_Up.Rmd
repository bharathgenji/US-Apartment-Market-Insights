...
author: "Zachary Perry, Bharath Genji Mohana Ranga, Erica Zhao, Chinmay Dhamapurkar"
date: `r Sys.Date()`
...
## 4. Initial Analyses and Findings
...
An important aspect of our analysis was to assess the significance of using the states column to group listings. The real estate market is extremely localized. To validate this approach, we used an ANOVA test, the results of which are shared in the subsequent section.
...
Deeper into this a regression of sq_ft ~ price for TX, CA, and NY can be taken to show the `ocational premium` or the amount of price determined by the location as opposed to the square footage. 
...
In analyzing real estate data, outliers can significantly distort the overall understanding of the dataset. Prices that are either exceptionally high or low, compared to the majority, can skew our average calculations and give a misleading portrayal of the market. Thus, our primary aim in this segment of the analysis is to identify and address outliers in the rental prices from the dataset.
...
apartments_data_clean <- read_excel("cleaned_data.xlsx")
...
```
The histogram showcases the distribution of rental prices devoid of outliers. It provides a more centralized view, allowing for easier identification of the mode and median prices, and offers a clearer understanding of where the majority of rental prices lie within the market.


#### Question

Listing Sources Analysis: Our primary aim in this analysis is to investigate how rental prices fluctuate based on the source of the listing. Sources like RentLingo, Zillow, among others, could influence the perceived value and, consequently, the listed rental price of properties. 



```{r, echo = FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.width=10, fig.height=4}
apartments_data<- read_excel("cleaned_data.xlsx")
...
# d. Visualization
ggplot(source_stats, aes(x = reorder(source, -price), y = price)) +
...
```
Upon completion of our analysis, several key insights have emerged regarding the variation in rental prices based on the listing source:

Descriptive Variations: Different listing sources exhibit distinct average rental prices. For instance, while sources like Listanza and RENTOCULAR lean towards higher average rental prices, others like GoSection8 and ListedBuy have averages that are relatively moderate.

Data Distribution: Our normality assessment revealed that rental prices for several sources do not adhere to a normal distribution. This is an essential observation, impacting the way we interpret and make subsequent decisions based on our data.

Statistical Differences: The ANOVA analysis confirmed that there are statistically significant differences in rental prices across various listing sources. This underscores the importance of considering the listing platform when evaluating rental price benchmarks.

Visual Insights: The bar chart provided a lucid representation of rental prices across sources, with variations clearly evident. The error bars further added context by showcasing the variability in prices for each source.

In essence, our analysis has illuminated the nuanced ways in which rental prices differ based on listing platforms. For potential renters, this emphasizes the importance of shopping around and exploring multiple platforms before making a decision. For property managers and landlords, understanding these platform-specific nuances can guide pricing strategies and listing decisions.

#### Analysis of Bedroom and Bathroom Count Against Price

Rental prices can vary significantly based on the structure and facilities of an apartment. Two of the most influential factors that renters consider are the number of bedrooms and bathrooms in a property. In this segment of our analysis, we aim to explore how these two key features influence rental prices.

Data Refinement: By selecting relevant columns and ensuring accurate data types, we lay a strong foundation for our analysis.

Combination Insights: Creating a combined metric like "2B1Ba" (indicating 2 bedrooms and 1 bathroom) offers a composite view, enabling us to analyze how different configurations influence rental prices.

Emphasis on Commonality: To derive meaningful insights, our focus will be on bedroom-bathroom combinations that are most common in the dataset.

Pricing Patterns: We'll derive average rental prices for each of these frequent combinations to discern any patterns or trends.

Visualization: A bar chart will be used to succinctly represent the average rental prices for these combinations, making the data more interpretable.
...
print(p1)
```
The resulting bar chart elucidates the fact that different bedroom-bathroom configurations have varied average rental prices. Certain combinations, possibly owing to their popularity, demand or scarcity, command a higher average rental price. The chart allows for a clear comparison between different configurations. Arranged in descending order of average price, it's evident which combinations are considered premium and which ones are more economical.

In summation, the configuration of an apartment, specifically the number of bedrooms and bathrooms, plays a pivotal role in its rental price. This analysis offers a granular understanding of such trends, proving invaluable for potential renters and landlords alike.

## Limitatations of The Dataset

SMART question 12 was cut from the analysis due to a data limitation. Due to the "sparse" observation count and high level of amenity bundling we received counter intuitive results such as a pool having a negative effect on price.
